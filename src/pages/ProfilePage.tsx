...
import * as z from "zod";
import { motion, useMotionValue, useTransform, useSpring, AnimatePresence } from "framer-motion";
import { User, Lock, Eye, EyeOff, ArrowLeft, Camera, Save, Loader2, ShieldCheck, AlertCircle, Sparkles, Heart, Star, Zap } from "lucide-react";
...
// --- Decorative Anime Components ---
const FloatingPetal = ({ delay = 0, x = "50%" }) => (
  <motion.div
    ...
    className="absolute pointer-events-none text-pink-300/30"
    style={{ left: x }}
  >
...
// --- 3D Tilt Component ---
const TiltCard = ({ children, className = "", glowColor = "rgba(255, 121, 198, 0.3)" }: { children: React.ReactNode; className?: string; glowColor?: string }) => {
  const x = useMotionValue(0);
  const y = useMotionValue(0);
  const mouseX = useSpring(x, { stiffness: 500, damping: 30 });
  const mouseY = useSpring(y, { stiffness: 500, damping: 30 });
...
export default function ProfilePage() {
  ...
  // --- State ---
  const [user, setUser] = useState(() => JSON.parse(localStorage.getItem("user") || '{"name": "Kira Yoshikage", "email": "kira@morioh.town", "profileImage": ""}'));
  const [photo, setPhoto] = useState<string>(user?.profileImage || "");
  const [preview, setPreview] = useState<string>(user?.profileImage || "");
  const [loadingPhoto, setLoadingPhoto] = useState(false);
  const [msg, setMsg] = useState("");
  ...
  const onSubmitPassword = async (data: PasswordFormValues) => {
    ...
    setErr("");
    try {
      // Mocking API call for demo purposes
      await new Promise(r => setTimeout(r, 1500));
      setMsg("Power level increased! (Password changed) ✨");
      reset();
    } catch (e: any) {
      setErr("The villain blocked your update!");
    }
  };
  ...
  const savePhoto = async () => {
    ...
    try {
      ...
      setUser(updatedUser);
      setMsg("Avatar evolution complete! 🌟");
    } catch (e: any) {
      setErr("Failed to synchronize avatar");
    } finally {
      setLoadingPhoto(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 selection:bg-pink-500/30 overflow-hidden relative font-sans">
      {/* Anime Background Patterns */}
      <div className="absolute inset-0 z-0 opacity-20 pointer-events-none overflow-hidden">
        <div className="absolute inset-0" style={{ backgroundImage: 'radial-gradient(#ff79c6 1px, transparent 1px)', backgroundSize: '30px 30px' }} />
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-purple-900/40 via-transparent to-pink-900/40" />
        {[...Array(8)].map((_, i) => (
          <FloatingPetal key={i} delay={i * 2} x={`${10 + i * 12}%`} />
        ))}
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-8 lg:py-16">
        ...
        >
          <button
            onClick={() => nav(-1)}
            className="group flex items-center gap-2 text-pink-400 hover:text-pink-300 transition-all mb-4 uppercase tracking-tighter font-black text-sm"
          >
            <div className="p-2 rounded-full bg-pink-500/10 group-hover:bg-pink-500/20 border border-pink-500/20">
              <ArrowLeft size={18} />
            </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end gap-4">
            <h1 className="text-5xl md:text-7xl font-black italic text-transparent bg-clip-text bg-gradient-to-r from-pink-400 via-purple-400 to-cyan-400 tracking-tighter leading-none">
              USER_PROFILE
            </h1>
            <div className="px-3 py-1 bg-cyan-500 text-black font-black text-xs uppercase skew-x-[-12deg] flex items-center gap-1 self-start md:mb-2">
              <Zap size={14} fill="currentColor" /> S-Rank Member
            </div>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4">
            <TiltCard className="h-full">
              <div className="bg-slate-900/80 backdrop-blur-md border-2 border-pink-500/30 rounded-[2.5rem] p-8 flex flex-col items-center shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-10">
                  <Star size={80} fill="white" />
                </div>

                <div className="relative mb-8">
                  <div className="absolute -inset-4 bg-gradient-to-tr from-pink-500 via-purple-500 to-cyan-500 rounded-full animate-spin-slow opacity-75 blur-md" />
                  <div className="relative w-40 h-40 rounded-full border-4 border-slate-900 overflow-hidden bg-slate-800">
                    <img
                      src={preview || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.name}`}
                      alt="Profile"
                ...
                <div className="text-center space-y-2">
                  ...
                  <p className="text-cyan-400 font-mono text-sm tracking-widest">{user?.email}</p>
                </div>
                ...
                <AnimatePresence>
                  {hasUnsavedPhoto && (
                    <motion.button
                      ...
                      onClick={savePhoto}
                      disabled={loadingPhoto}
                      className="mt-8 w-full py-4 bg-pink-500 hover:bg-pink-400 text-white rounded-2xl font-black uppercase tracking-widest shadow-[0_0_20px_rgba(236,72,153,0.4)] transition-all flex items-center justify-center gap-2"
                    >
                      {loadingPhoto ? <Loader2 className="animate-spin" size={20} /> : <><Save size={20} /> Sync Changes</>}
                    </motion.button>
                  )}
                </AnimatePresence>

                <div className="mt-12 w-full space-y-4">
                  <div className="flex justify-between items-center p-4 bg-slate-800/50 rounded-2xl border border-white/5">
                  ...
                  <div className="flex justify-between items-center p-4 bg-slate-800/50 rounded-2xl border border-white/5">
                    <span className="text-slate-400 font-bold text-xs uppercase">Loyalty</span>
                    <div className="flex gap-1">
                      {[...Array(5)].map((_, i) => <Star key={i} size={14} fill={i < 4 ? "#fbbf24" : "none"} className={i < 4 ? "text-yellow-400" : "text-slate-600"} />)}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <AnimatePresence mode="wait">
              {(msg || err) && (
                <motion.div
                  ...
                  className={`p-5 rounded-3xl border-2 flex items-center gap-4 shadow-2xl ${msg ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-300' : 'bg-rose-500/10 border-rose-500/50 text-rose-300'}`}
                >
                  <div className={`p-3 rounded-2xl ${msg ? 'bg-emerald-500/20' : 'bg-rose-500/20'}`}>
                    {msg ? <Sparkles size={24} /> : <AlertCircle size={24} />}
                  </div>
                  <span className="font-black uppercase tracking-tight">{msg || err}</span>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <TiltCard glowColor="rgba(139, 92, 246, 0.3)">
              <div className="bg-slate-900/80 backdrop-blur-md border-2 border-purple-500/30 rounded-[2.5rem] p-8 md:p-10">
                <div className="flex items-center gap-4 mb-10">
                  <div className="p-4 bg-purple-500/20 text-purple-400 rounded-2xl border border-purple-500/30">
                    <Lock size={28} />
                  </div>
                  <div>
                    <h3 className="text-2xl font-black text-white italic tracking-tighter uppercase">Security Protocol</h3>
                    <p className="text-slate-500 font-medium">Rotate your encryption keys</p>
                  </div>
                </div>

                <form onSubmit={handleSubmit(onSubmitPassword)} className="space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      <label className="text-xs font-black uppercase tracking-[0.2em] text-purple-400 ml-1">Current Key</label>
                      <div className="relative">
                        <input
                          type={showCurrent ? "text" : "password"}
                          {...register("currentPassword")}
                          className="w-full h-14 bg-slate-800/50 border-2 border-slate-700 rounded-2xl px-5 pr-14 outline-none focus:border-purple-500 transition-all font-mono text-lg"
                          placeholder="••••••••"
                        />
                        <button
                          type="button"
                      ...
                      {errors.currentPassword && <p className="text-xs text-rose-400 font-bold ml-2 uppercase italic">{errors.currentPassword.message}</p>}
                    </div>

                    <div className="space-y-3">
                      <label className="text-xs font-black uppercase tracking-[0.2em] text-purple-400 ml-1">New Key</label>
                      <div className="relative">
                        <input
                          type={showNew ? "text" : "password"}
                          {...register("newPassword")}
                          className="w-full h-14 bg-slate-800/50 border-2 border-slate-700 rounded-2xl px-5 pr-14 outline-none focus:border-purple-500 transition-all font-mono text-lg"
                          placeholder="NEW_PW_01"
                        />
                        <button
                          type="button"
                          onClick={() => setShowNew(!showNew)}
                      ...
                      {errors.newPassword && <p className="text-xs text-rose-400 font-bold ml-2 uppercase italic">{errors.newPassword.message}</p>}
                    </div>
                  </div>
                  ...
                  <div className="pt-4 flex justify-end">
                    <motion.button
                      ...
                      type="submit"
                      disabled={isSubmitting}
                      className="px-10 py-4 bg-white text-black rounded-2xl font-black uppercase tracking-tighter shadow-[5px_5px_0px_#8b5cf6] hover:shadow-[2px_2px_0px_#8b5cf6] transition-all disabled:opacity-50 flex items-center gap-3"
                    >
                      {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : "Initialize Update"}
                    </motion.button>
                  </div>
                </form>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <motion.button 
                whileHover={{ y: -5 }}
                className="p-6 bg-cyan-500/10 border-2 border-cyan-500/30 rounded-[2rem] flex items-center justify-between group hover:bg-cyan-500/20 transition-all"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    <h4 className="font-black text-white uppercase italic text-sm">Bio Data</h4>
                    <p className="text-xs text-cyan-400 font-bold">Modify Identity</p>
                  </div>
                </div>
              </motion.button>
              ...
              <motion.button 
                whileHover={{ y: -5 }}
                className="p-6 bg-pink-500/10 border-2 border-pink-500/30 rounded-[2rem] flex items-center justify-between group hover:bg-pink-500/20 transition-all"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    <h4 className="font-black text-white uppercase italic text-sm">Preferences</h4>
                    <p className="text-xs text-pink-400 font-bold">Custom Settings</p>
                  </div>
                </div>
              </motion.button>
            </div>
